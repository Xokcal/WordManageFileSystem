...
public class MsgImpl {
    ...
    public Integer addUserMsg(Integer userId, MsgBody msgBody) {
        if (CheckValidUtil.isNull(userId) || CheckValidUtil.isNull(msgBody)) {
            ...
            throw new RuntimeException("添加用户基础信息，用户id或传入数据出问题！！");
        }
        ...
        throw new RuntimeException("添加用户基础信息，失败！！");
    }
    ...
    public Integer updateOrInsertByExists(Integer userId, MsgBody msgBody) {
        if (CheckValidUtil.isNull(userId)) {
            ...
            throw new RuntimeException("更新用户基础信息，用户id出问题！！");
        }
        if (isUserMsgExists(userId)) { //存过
            System.out.println("=========================存过========================");
            ...
            Integer r = chatMapper.updateBeAddFriendMsg(
                    msgBody.getName()
                    , userId);
            if (insertResult > 0) {
                log.info("更新用户基础信息，更新对方好友信息，成功！！");
                return 1;
            }
            if (r < 0){
            ...
            if (msgBody == null) { //信息等于null时
                ...
                throw new RuntimeException("更新用户基础信息，传空值失败！！");
            }
            //没存过，但是有信息（）信息不等于null
            ...
            throw new RuntimeException("更新用户基础信息，没存过时，没用执行任何分支逻辑！");
        }
        throw new RuntimeException("更新用户基础信息方法没执行任何分支逻辑！");
    }
    ...
    public Integer msgImgUpload(Integer userId, MultipartFile img) {
        if (CheckValidUtil.isNull(userId) || CheckValidUtil.isNull(img)) {
            ...
            throw new RuntimeException("头像上传 , 用户名或头像文件为空值！");
        }
        if (isUserMsgExists(userId)) { //存在数据
            ...
            throw new RuntimeException("头像上传，数据库上传失败！！");
        } else { //不存在，先创建，再单独上传
            Integer integer = msgMapper.msgNullInsert(userId); //全null插入
            ...
            log.error("头像上传，创建失败！！");
            throw new RuntimeException("头像上传，创建失败！！");
        }
    }
